...
# 1. SETUP DE SEGURANÇA E INICIALIZAÇÃO (IMPEDE O ERRO ATTRIBUTEERROR)
...
if 'zona' not in st.session_state: st.session_state.zona = "Analise"

# 2. IDENTIDADE VISUAL LIDERUM
...
st.markdown("""
    ...
    .stButton>button { 
        ...
        color: #001226 !important; width: 100%; font-weight: bold; padding: 18px; border: none; border-radius: 8px;
    }
...
# 3. LISTA INTEGRAL DAS 45 PERGUNTAS
questoes_lista = [
        ...
        with st.form("lead_form"):
            ...
            if st.form_submit_button("LIBERAR ACESSO AO RESULTADO"):
                if all([nome, email, whatsapp, cargo]):
                    ...
                    payload = {"nome": nome, "email": email, "whatsapp": whatsapp, "cargo": cargo, "pontos": t, "zona": st.session_state.zona}
                    try:
...
# --- ETAPA 3: LAUDO E CHECKOUT (A ISCA PAGA) ---
elif st.session_state.etapa == 'resultado':
    st.markdown(f"## Análise de Performance: {st.session_state.nome_usuario}")
    ...
    with col_r:
        ...
        if st.session_state.zona == "ELITE": st.success("Governança de alto nível. Foco em manutenção.")
        elif st.session_state.zona == "OSCILAÇÃO": st.warning("Performance inconstante. Necessário estabilizar processos.")
        else: st.error("Estado Crítico. Urgência em recuperar disciplina.")
