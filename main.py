...
# 1. SETUP VISUAL LIDERUM (NOMES EM BRANCO PARA VISIBILIDADE)
...
st.markdown("""
    ...
    .stButton>button { 
        ...
        color: #001226 !important; width: 100%; font-weight: bold; padding: 15px; 
    }
...
if st.session_state.etapa == 'questoes':
    # ATALHO PARA TESTE (Opcional, para você validar rápido)
    ...
    if st.button("PROCESSAR MEU DIAGNÓSTICO"):
        if sum(1 for i in range(45) if st.session_state.get(f"q_{i}") is not None) == 45:
            ...
            st.session_state.etapa = 'captura'
            st.rerun()
        else: st.error("⚠️ Por favor, responda todas as 45 questões antes de continuar.")

# ETAPA 2: CAPTURA E GRAVAÇÃO (FORMA NATIVA)
        ...
        with st.form("lead_form"):
            ...
            if st.form_submit_button("LIBERAR MEU RESULTADO"):
                if all([nome, email, whatsapp, cargo]):
                    ...
                    try:
                        # CONEXÃO NATIVA: Lê direto do Secrets corrigido
