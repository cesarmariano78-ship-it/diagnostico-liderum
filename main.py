...
# INICIALIZAÇÃO DE ESTADO (IMPEDE ERROS DE MEMÓRIA)
...
# URL QUE VOCÊ FORNECEU (image_dd0739.png)
...
# 2. LISTA DAS 45 PERGUNTAS (9 DIMENSÕES)
questoes_lista = [
...
# --- ETAPA 3: RESULTADO E LAUDO ---
    ...
    with col_l:
        ...
        fig.update_layout(polar=dict(bgcolor="rgba(0,12,26,1)", radialaxis=dict(visible=True, range=[0, 25], color="white")), showlegend=False, paper_bgcolor="rgba(0,0,0,0)")
    ...
    with col_r:
        ...
        if st.session_state.zona == "ELITE": st.markdown("**DIRECIONAMENTO:** Governança de alto nível. O desafio é a manutenção.")
        elif st.session_state.zona == "OSCILAÇÃO": st.markdown("**DIRECIONAMENTO:** Performance inconstante. Necessário estabilizar pilares.")
        else: st.markdown("**DIRECIONAMENTO:** Estado crítico. Intervenção imediata necessária.")
    ...
    if st.button("RECOMEÇAR DIAGNÓSTICO"): st.session_state.etapa = 'questoes'; st.rerun()
